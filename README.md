...
- Patrones cloud-native de GCP: Firestore para estado rápido + event sourcing, un único worker Pub/Sub→Cloud Run para todos los países (sin duplicar infraestructura donde no hay diferencia de comportamiento), Cloud SQL para el lado relacional durable, Secret Manager + IAM de mínimo privilegio
