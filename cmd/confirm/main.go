...
func main() {
	...
	// Complete (this binary's only call) never touches the publisher/relationalRepo/
	// confirmationPublisher ports - those are exercised by cmd/api's Create and
	// cmd/worker's Persist respectively.
	...
	log.Printf("confirm listening on :%s", port)
