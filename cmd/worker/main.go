...
func main() {
	...
	fsClient, cleanup, err := shared.Bootstrap(ctx, gcpProject, "gcp-go-worker")
	if err != nil {
	...
	confirmationPublisher := newConfirmationPublisher(psClient)
...
func cloudSQLDSN() string {
	host := shared.MustEnv("CLOUDSQL_SOCKET_DIR")
	user := shared.MustEnv("CLOUDSQL_USER")
	db := shared.MustEnv("CLOUDSQL_DB")
	password := shared.MustEnv("CLOUDSQL_PASSWORD")
