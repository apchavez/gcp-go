...
func (h *Handlers) CreateAppointment(w http.ResponseWriter, r *http.Request) {
	...
	if req.InsuredID == "" || req.ScheduleID == 0 {
		shared.Bad(w, "insuredId and scheduleId are required")
		return
	}
	...
	if err != nil {
		slog.Error("CreateAppointment failed", "insuredId", req.InsuredID, "scheduleId", req.ScheduleID, "error", err)
		shared.Internal(w, "")
		return
	}
	slog.Info("appointment created", "insuredId", req.InsuredID, "appointmentUuid", appointment.AppointmentUUID)
...
func (h *Handlers) ListByInsured(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Error("ListByInsured failed", "insuredId", insuredID, "error", err)
		shared.Internal(w, "")
		return
	}
	shared.OK(w, page)
...
func (h *Handlers) GetAppointmentHistory(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Error("GetByID failed", "appointmentUuid", appointmentUUID, "error", err)
		shared.Internal(w, "")
		return
	}
	if appointment == nil {
	...
	if err != nil {
		slog.Error("GetHistory failed", "appointmentUuid", appointmentUUID, "error", err)
		shared.Internal(w, "")
		return
	}
	shared.OK(w, events)
