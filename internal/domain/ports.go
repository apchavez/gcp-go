...
// AppointmentConfirmationPublisher mirrors EventBridgeConfirmationBus (AWS sibling,
// src/infra/messaging/eventbridge.service.ts) - after the relational persist stage, it
// publishes a confirmation event that Eventarc routes to a separate handler which marks
// the aggregate COMPLETED and notifies. This decouples "persist to the relational store"
// from "mark completed + notify" the same way EventBridge -> SQS -> confirmAppointment λ
// does on AWS, instead of one worker doing both steps inline.
type AppointmentConfirmationPublisher interface {
	PublishConfirmed(ctx context.Context, appointmentUUID string) error
}
