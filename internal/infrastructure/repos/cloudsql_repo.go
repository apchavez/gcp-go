...
const upsertSQL = `
INSERT INTO appointments (appointment_uuid, insured_id, schedule_id, status, created_at, updated_at)
VALUES ($1, $2, $3, $4, $5, $6)
...
func (r *CloudSQLRepo) Persist(ctx context.Context, a domain.Appointment) error {
	return r.res.Run(ctx, func() error {
		_, err := r.pool.Exec(ctx, upsertSQL,
			a.AppointmentUUID, a.InsuredID, a.ScheduleID, a.Status, a.CreatedAt, a.UpdatedAt,
		)
		return err
	})
}
