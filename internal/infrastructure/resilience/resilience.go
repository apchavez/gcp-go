...
func New(name string) *Resilience {
	return &Resilience{name: name, breaker: NewCircuitBreaker(name)}
}

// Run traces the call as a span named after this Resilience's component (e.g.
// "firestore-state-repo") before applying retry/circuit-breaker - every infra call funnels
// through here, so this single wrapper instruments all of them at once, the same way the
// AWS sibling's captureAWSClient instruments a client once instead of per call site.
func (r *Resilience) Run(ctx context.Context, fn func() error) error {
	ctx, end := tracing.StartSpan(ctx, r.name)
